...
export default function TrHero({id}) {
    return (
        <section className="w-full aspect-video lg:aspect-[16/3] flex flex-col py-8 items-start justify-end px-4 md:px-8 lg:px-16 xl:px-32 2xl:px-48 relative">
            ...
            <Image src={`https://picsum.photos/seed/${id}/1920/360`} fill={true} className="object-cover object-center z-0" />
            <div className="w-fit flex flex-col gap-2 z-20">
                <h1 className="text-[#FCFCFC] text-3xl xl:text-4xl 2xl:text-5xl font-bold">
                    {/* {data?.event?.eventName} */}
                    {id}
                </h1>
                <div className="w-full h-[3px] bg-pac-orange rounded-full" />
                <h5 className="text-[#EFEFEF] 2xl:text-lg">
                    {/* {data?.event?.location} */}
                    {id}
                </h5>
                <h5 className="text-[#EFEFEF] 2xl:text-lg">
