                    ...
                    <div key={i} className="h-fit w-full rounded-md">
                        <a href={data ? `/news-&-updates/${idFormatter(n?.node?.id, true)}` : ``} className="w-fit h-fit">
                            <div className="w-full four-to-three relative">
                                <Image src={data ? sortByAttribute(n?.node?.mediaLine, 'title')[0].sourceUrl : ``} alt={data ? n?.node?.mediaLine[0].altText : ``} fill={true} className="object-contain object-center" />
                            </div>
                        </a>
                        <a href={data ? `/news-&-updates/${idFormatter(n?.node?.id, true)}` : ``} className="w-full landscape-banner p-2">
                            <h4 className="text-[#121212] font-semibold transition-all ease-in-out duration-300 hover:underline">{n?.node?.name}</h4>
                        </a>
                    </div>
                )) : ``}
            </div>
