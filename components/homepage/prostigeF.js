...
export default function ProstigeF(props) {
  ...
  return (
    <section className="flex flex-col items-center justify-center h-screen 2xl:h-fit max-h-fit w-full gap-8 xs:py-20 lg:py-16 xs:px-4 lg:px-32 2xl:px-48">
      <div className="w-full h-fit flex xs:flex-col-reverse lg:flex-row xs:justify-center lg:justify-between 2xl:justify-around xs:items-center lg:items-center lg:gap-12 xs:gap-24">
        <div className="md:max-w-lg max-w-full flex flex-col gap-8 ">
          <div className="flex flex-col justify-end gap-2 relative w-80 h-[96px]">
                ...
                map((c, i) => (
                  <div key={i} className="w-32 flex flex-col gap-2 items-center">
                    <span className="w-20 h-20 object-fill relative">
                      <Image src={data?.homepageSections?.nodes[0] ? sortByAttribute(data?.homepageSections?.nodes[0].mediaLine2, 'title')[i].link : ``} fill={true} className="object-contain" />
                    </span>
          ...
          <div className="flex flex-col gap-2 w-full">
            <div className="w-full h-[2px] rounded-full bg-nav-green" />
            <div className="flex flex-col gap-1 w-full">
              <h2 className="text-xl text-pac-orange font-bold uppercase">{data?.homepageSections?.nodes[0].contentLine2[0]}</h2>
              <h4 className="text-[#373737] text-sm">{data?.homepageSections?.nodes[0].contentLine2[1]}</h4>
              <Button type={1} color={'orange'} name={data?.homepageSections?.nodes[0].contentLine2[2]} link={'/#'} />
            </div>
          </div>
        </div>
        <div className="xs:w-64 xs:h-36 lg:w-[448px] lg:h-[280px] 2xl:w-[512px] 2xl:h-[320px] rotate-12 shadow-md rounded-3xl overflow-hidden">
