...
export default function CompanyActivitiesSlider({ media, index }) {
    ...
    return (
        <section className='relative w-full h-fit flex items-center justify-center'>
            <button className={`p-2 flex items-center justify-center hover:shadow-md shadow-sm bg-[#FCFCFC]/75 hover:bg-[#FCFCFC]/100 rounded-full absolute ${index%2 === 0 ? `left-8` : `right-8`} bottom-8 z-30 transition-all`} onClick={scrollNext}>
