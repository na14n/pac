...
export const BrandSliderF = ({ brands, mediaUrl, chunk }) => {
    ...
    return (
        <div className='max-md:w-full test w-fit h-full lg:px-32 2xl:px-48 flex items-center justify-center py-16 max-lg:py-8 bg-red-500'>
            ...
            <div className="embla__viewport self-center test h-full xs:w-[15rem] md:w-[30rem] lg:w-[38rem] 2xl:w-[46rem] flex justify-center items-center `" ref={emblaRef}>
                {/* Between Slides */}
                <div className="h-full w-fit touch-pan-y flex gap-8 z-40 py-4 px-8">
                    {arrayOfArrays.map((w, index) => (
                        <div key={index} className=' xs:flex-[0_0_14rem] md:flex-[0_0_28rem] lg:flex-[0_0_36rem] 2xl:flex-[0_0_44rem] min-w-0 relative h-full grid xs:grid-auto-fit-[6rem] lg:grid-auto-fit-[6rem] 2xl:grid-auto-fit-[8rem] gap-2 justify-items-center z-40'>
                            {w.map((c, index) => (
                                <a key={index} className=' bg-[#EEE] w-28 md:w-[5rem] lg:w-24 xl:w-28 2xl:w-32 aspect-square p-2 flex flex-col justify-center items-center rounded-md hover:-translate-y-1 hover:shadow-md transition-all border-2 border-[#EEE] hover:border-pac-green relative z-40 overflow-hidden' href={`/brands/${slugFormatter(c.name, false, true)}`}>
