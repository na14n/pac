...
export const BrandSliderF = ({ brands, mediaUrl }) => {
    ...
    return (
        <div className="embla h-content w-content overflow-hidden z-30 relative lg:px-4 flex justify-center items-center">
            <div className='absolute flex justify-between items-center w-full h-content'>
            ...
            <div className="embla__viewport self-center h-content w-full " ref={emblaRef}>
                <div className="embla__container h-full w-full ">
                    {arrayOfArrays.map((w, index) => (
                        <div key={index} className='embla__slide w-full h-full grid lg:grid-cols-5 gap-4 py-8 justify-items-center items-center px-16'>
                            {w.map((c, index) => (
                                <a key={index} className='bg-[#EEE] w-28 h-28 p-2 flex flex-col justify-center items-center rounded-md hover:-translate-y-1 hover:shadow-md transition-all border-2 border-[#EEE] hover:border-pac-green ' href='#'>
