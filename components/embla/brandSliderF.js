...
export const BrandSliderF = ({ brands, mediaUrl, chunk }) => {
    ...
    return (
        <div className="embla h-fit xs:w-full lg:w-fit overflow-hidden z-30 relative lg:px-4 px-0 flex justify-center items-center ">
            ...
            <div className="embla__viewport self-center h-fit w-full " ref={emblaRef}>
                <div className="embla__container h-full w-full ">
                    {arrayOfArrays.map((w, index) => (
                        <div key={index} className='embla__slide w-full h-full grid xs:grid-cols-2 lg:grid-cols-5 gap-4 2xl:gap-8 py-8 justify-items-center items-center xs:px-12 lg:px-16 2xl:px-24'>
                            {w.map((c, index) => (
