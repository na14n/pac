...
const CareerItem = ({ title, location, tags, requirement, description, benefits }) => {
    ...
    return (
        <div className={`lg:w-[700px] xs:w-full h-fit shadow-lg rounded-md flex flex-col justify-between relative overflow-hidden`}>
            <div className="w-full h-fit flex gap-4 items-center justify-between rounded-md overflow-hidden relative z-40 bg-white">
                <div className="p-4 h-full absolute top-0 left-0 bg-[#777] shrink-0 z-10 flex items-center justify-center">
                    <Icon icon="mdi:account-circle-outline" className="text-white font-bold text-2xl" />
                </div>
                <div className="flex w-full gap-2 items-center ml-20">
                    <h3 className="text-lg uppercase h-full w-fit text-pac-orange py-4 font-bold">
                        {title ? title : 'Job Title'}
                    </h3>
                    <span className="max-md:hidden flex w-fit gap-1 bg-pac-green text-white shrink-0 p-1 px-2 rounded-md items-center">
                ...
                <div className="flex gap-1 items-center w-fit px-4">
                    {tags ? tags.map((t, index) => (
                        <span key={index} className="max-md:hidden flex gap-1 bg-nav-orange/25 text-pac-orange shrink-0 p-1 px-2 rounded-md items-center">
            ...
            <div className={` w-full bg-[#FCFCFC] px-8 z-0 rounded-md transition-all top-0 ease-in-out duration-300 overflow-y-auto scrollbar ${accordionState === false ? 'max-h-0' : 'max-h-96'}`}>
                <div className="mt-8 flex flex-wrap md:hidden gap-2 items-center w-fit">
