...
export default function CareersSection2() {
    ...
    return (
        <section className="w-full h-fit flex flex-col items-center">
            <h2 className="text-3xl 2xl:text-5xl font-bold text-pac-green py-12">
                {data?.contactUsContents?.nodes[0]?.sectionHeading}
            </h2>
            <div className="w-full grid grid-cols-4 px-4 md:px-8 lg:px-16 xl:px-32 2xl:px-48 gap-y-8">
                <div className="w-full aspect-[3/4] bg-[#ffdfb9] p-8 flex items-center justify-center">
                    <span className="w-fit h-fit">
                        <h5 className="text-lg 2xl:text-xl text-pac-orange max-w-[16ch] font-semibold mb-2">
                            {data?.contactUsContents?.nodes[0]?.sectionSubheading[0]}
                        </h5>
                        <div className="w-full h-[2px] rounded-full bg-pac-orange" />
                    </span>
                </div>
                <div className="w-full aspect-[3/4] p-8 flex flex-col gap-8 items-center justify-center">
                    <span className="text-sm">{parse(data?.contactUsContents?.nodes[0]?.contentLine1[0])}</span>
                    <div className="flex items-center gap-4">
                        <span className="w-16 h-16 rounded-full overflow-hidden relative">
                        ...
                        <span className="flex flex-col justify-center">
                            ...
                            <p className="text-xs text-[#272727]">{data?.contactUsContents?.nodes[0]?.textLine2[0]}</p>
                        </span>
                    </div>
                </div>
                <div className="w-full aspect-[3/4] bg-[#bfe9c3] p-8 flex items-center justify-center">
                    <span className="w-fit h-fit">
                        <h5 className="text-lg 2xl:text-xl text-pac-green max-w-[16ch] font-semibold mb-2">
                ...
                <div className="w-full aspect-[3/4] p-8 flex flex-col gap-8 items-center justify-center">
                    <span className="text-sm">{parse(data?.contactUsContents?.nodes[0]?.contentLine1[1])}</span>
                    <div className="flex items-center gap-4">
                        <span className="w-16 h-16 rounded-full overflow-hidden relative">
                ...
                <div className="w-full aspect-[3/4] p-8 flex flex-col gap-8 items-center justify-center">
                    <span className="text-sm">{parse(data?.contactUsContents?.nodes[0]?.contentLine1[2])}</span>
                    <div className="flex items-center gap-4">
                        <span className="w-16 h-16 rounded-full overflow-hidden relative">
                ...
                <div className="w-full aspect-[3/4] p-8 flex flex-col gap-8 items-center justify-center">
                    <span className="text-sm">{parse(data?.contactUsContents?.nodes[0]?.contentLine1[3])}</span>
                    <div className="flex items-center gap-4">
                        <span className="w-16 h-16 rounded-full overflow-hidden relative">
            ...
            <div className="w-full h-fit my-8 relative overflow-hidden" ref={emblaRef}>
                <div className="flex touch-pan-y gap-2 shadow-md w-fit" >
                    {data ? sortByAttribute(data?.contactUsContents?.nodes[0]?.mediaLine2).map((a, index) => (
                        <div key={index} className='rounded-sm flex-[0_0_8rem] sm:flex-[0_0_12rem] md:flex-[0_0_32rem] lg:flex-[0_0_20rem] xl:flex-[0_0_24rem] min-[1700px]:flex-[0_0_32rem] aspect-[3/2] relative min-h-0'>
