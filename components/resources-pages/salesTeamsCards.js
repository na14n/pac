...
export default function SalesTeamsCards() {
    ...
    return (data ?
        sortByAttribute(data?.salesAgentsTeams?.nodes, "position").map((d, index) => (
            <a
                key={index}
                className="w-full h-fit flex flex-col gap-2 border-2 hover:first:border-pac-orange first:text-pac-orange hover:border-pac-green text-pac-green hover:last:border-[#323232] last:text-[#373737] hover:shadow-lg border-[#DDD]/0 transition-all p-2 group hover:-translate-y-2"
