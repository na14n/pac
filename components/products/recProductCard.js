...
const RecommendedProductCard = ({ media, name, brand, category, slug, best }) => {

    const breadcrumbs = category ? breadcrumbsFormatter(category) : {}

    return (
        <a className="z-10 rounded-sm overflow-hidden bg-[#FCFCFC] shadow-sm w-full aspect-[3/4] relative  flex flex-col items-center justify-end group hover:-translate-y-1 hover:shadow-lg transition-all" href={slug ? `/products/${slug}` : '/'}>
            ...
            <div className="w-full aspect-square shrink-0 z-0 relative flex items-center justify-center overflow-hidden bg-[#FFF]">
            ...
            <div className="w-full h-full flex flex-col px-4 py-2 z-10">
                <div className="font-bold text-[#171717] text-xs xl:text-lg 2xl:hidden">{name ? name.length > 12 ? name.substring(0, 12) + "..." : name: ""}</div>
                <div className="font-bold text-[#171717] text-xs xl:text-lg hidden 2xl:block">{name ? name.length > 12 ? name.substring(0, 24) + "..." : name: ""}</div>
