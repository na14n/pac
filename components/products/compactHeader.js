    ...
    return (
        <section className="h-fit w-full relative flex flex-col items-center justify-end md:px-8 lg:px-16 xl:px-32 2xl:px-48 md:pt-32 gap-8 bg-[#F4F4F4]">
            ...
            <div className="w-full h-fit shadow-lg rounded-sm z-10 max-md:mt-24">
                ...
                <div className="w-full grid-cols-3 grid md:grid-cols-6 xl:grid-cols-12 bg-[#ec6501]">
                    {categories.map((c, index) => (
                        <a key={index} className="aspect-square p-2 flex flex-col items-center gap-1 group" href={`/categories?category=${c?.name.toLowerCase()}`}>
                            <span className="w-1/3 xl:w-3/5 square relative border-[1.5px] rounded-md border-white/25 group-hover:border-white/80">
                                <Image src={c?.icon?.link} fill={true} className="object-contain object-center p-1" />
                            </span>
                            <p className="text-[.9rem] font-semibold text-[#EFEFEF] w-full text-center group-hover:text-white">
                                {c?.name}
                            </p>
                        </a>
                    ))}
