...
const ProductCard = ({ media, name, brand, category, slug, best }) => {
    ...
    return (
        <a className="z-10 rounded-sm overflow-hidden bg-[#FCFCFC] shadow-sm w-48 aspect-[3/4] relative  flex flex-col items-center justify-end group hover:-translate-y-1 hover:shadow-lg transition-all" href={slug ? `/products/${slug}` : '/'}>
            ...
            <div className="w-48 h-48 shrink-0 max-lg:w-32 max-lg:h-32 z-0 relative flex items-center justify-center overflow-hidden bg-[#FFF]">
            ...
            <div className="w-full h-full flex flex-col px-4 py-2 z-10">
                <div className="font-bold text-[#171717] text-lg">{name ? name.length > 12 ? name.substring(0, 12) + "..." : name: ""}</div>
